"""
Training script with Stable Mixed Precision (AMP) support
Drop-in replacement for train.py with AMP optimizations
"""
import os
import argparse
import logging
...
from managers.trainer_amp_stable import StableAMPTrainer
...
def main(params):
    ...
    if hasattr(params, 'db_path') and params.db_path:
        # Use user-provided path (can be absolute or relative)
        if not os.path.isabs(params.db_path):
            params.db_path = os.path.join(params.main_dir, params.db_path)
        logging.info(f"Using user-specified subgraph path: {params.db_path}")
    else:
        # Auto-generate path based on parameters
        ...
        logging.info(f"Auto-generated subgraph path: {params.db_path}")
    ...
    # Use Stable AMP Trainer
    trainer = StableAMPTrainer(params, graph_classifier, train, valid_evaluator)

    logging.info('Starting training with Stable Mixed Precision (AMP)...')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='GraIL Training with Stable AMP')
    ...
    parser.add_argument('--use_amp', action='store_true', default=True,
                        help='Enable Stable Mixed Precision Training (default: True)')
    parser.add_argument('--disable_amp', action='store_true',
                        help='Disable AMP (use FP32 only)')
    ...
    parser.add_argument('--db_path', type=str, default=None,
                        help='Custom path to pre-generated subgraph cache (LMDB). If not specified, path will be auto-generated. Can be absolute or relative to project root.')
    ...
    # Extraction optimization params
    # Negative samples caching for reproducible experiments
    parser.add_argument('--negatives_cache', type=str, default=None,
                        help='Path to cached negative samples (for reproducible experiments). If not exists, will be created and saved.')

    params = parser.parse_args()

    # Handle AMP flags
    if params.disable_amp:
